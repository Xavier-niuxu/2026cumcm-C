# -*- coding: utf-8 -*-
"""Linear fit predictor: use linear regression on past 4 same-weekday days."""
...
class LinearFitPredictor(BasePredictor):
    """Predict using linear regression on past 4 same-weekday days.
    
    For each time slot, fit a linear trend using the 4 historical values
    and extrapolate to predict the target date.
    """
    
    def predict(self, target_date: str) -> tuple:
        """Predict load and PV for target date using linear fit.
